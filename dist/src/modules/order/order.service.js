...
const createOrder = async (userId, address) => {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        for (const [pId, items] of Object.entries(itemsByProvider)) {
            const totalAmount = items.reduce((acc, item) => acc + item.meal.price * item.quantity, 0);
            const newOrder = await tx.order.create({
                data: {
                    totalAmount,
                    address,
...
const cancelOrderByCustomer = async (orderId, userId) => {
    ...
    if (order.status === OrderStatus.DELIVERED) {
        throw new Error("Cannot cancel order after it has been accepted or prepared.");
    }
...
const updateOrderStatusByProvider = async (orderId, status, userId) => {
    const provider = await prisma.providerProfile.findUnique({
        where: { userId },
    });
    const order = await prisma.order.findUnique({ where: { id: orderId } });
    ...
    if (order.status === OrderStatus.CANCELLED) {
        throw new Error("Cannot change order status after it has been cancelled or prepared.");
    }
    return await prisma.order.update({
        where: { id: orderId },
        data: { status },
